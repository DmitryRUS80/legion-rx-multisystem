...
const ClassicRCEngine=(()=>{
  ...
  function skipCurrentEvent(now=Date.now()){const ev=currentEvent();if(!ev)return{ok:false,error:'Нет текущего заезда'};const result=(ev.pilots||[]).map((id,i)=>({pilotId:id,status:'DNS',place:null,laps:0,timeMs:0,carNumber:i+1}));const fake={live:Object.fromEntries((ev.pilots||[]).map(id=>[id,{laps:0,bestLapMs:null,lastLapMs:null,lapTimes:[],timeMs:0,elapsedMs:0,started:false,finished:true}]))};saveHeat(ev.key,result,fake,now);return{ok:true,event:ev};}
  ...
  return Object.freeze({get,hasActive,create,updateSetup,prepare,persist,reset,archive,pilot,pilots,category,currentEvent,findEvent,nextEvent,startPilots,stageLabel,saveHeat,canStartScheduleHeat,startScheduleHeat,skipScheduleBreak,adjustScheduleBreak,addScheduleBreak,updateCurrentHeatSettings,skipCurrentEvent,pauseCompetition,resumeCompetition,rebuildSeeding,rebuildQualification,buildFinals,bestConsecutive});
})();
