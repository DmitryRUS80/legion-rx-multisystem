...
const ClassicRCEngine=(()=>{
  ...
  function heatScheduleKey(stage,round,heat,groupName=''){return groupName?`heat:${stage}:${groupName}:${Number(round)||1}`:`heat:${stage}:${Number(round)||1}:${Number(heat)||1}`;}
  function breakScheduleKey(stage,round=''){return`break:${stage}${round!==''?`:${round}`:''}`;}
  function scheduleDuration(key,fallback,kind='heat',settings=event?.settings||{}){const raw=Number(settings?.scheduleOverrides?.[key]),base=Number(fallback),min=kind==='break'?0:1,max=kind==='break'?240:120;if(Number.isFinite(raw))return Math.max(min,Math.min(max,raw));return Math.max(min,Math.min(max,Number.isFinite(base)?base:min));}
  function buildPreFinalEvents(){
    const c=category(),base=baseHeatGroups(),out=[];let order=0;
    for(let r=1;r<=event.settings.seedingRounds;r++)for(const g of base)out.push(makeHeat({stage:'seeding',round:r,heat:g.heat,pilots:g.pilots,label:`РАССТАНОВКА ${r} · ГРУППА ${g.heat}/${base.length}`,durationMin:scheduleDuration(heatScheduleKey('seeding',r,g.heat),c.raceMinutes,'heat'),order:++order}));
    for(let r=1;r<=event.settings.controlledPracticeRounds;r++)for(const g of base)out.push(makeHeat({stage:'controlled',round:r,heat:g.heat,pilots:g.pilots,label:`ПРАКТИКА ${r} · ГРУППА ${g.heat}/${base.length}`,durationMin:scheduleDuration(heatScheduleKey('controlled',r,g.heat),c.raceMinutes,'heat'),order:++order}));
    for(let r=1;r<=event.settings.qualifyingRounds;r++){
      const seq=classicRCHeatOrder(event.category,base.length,r);
      for(const h of seq){const g=base.find(x=>x.heat===h);out.push(makeHeat({stage:'qualifying',round:r,heat:h,pilots:g.pilots,label:`КВ${r} · ГРУППА ${h}/${base.length}`,durationMin:scheduleDuration(heatScheduleKey('qualifying',r,h),c.raceMinutes,'heat'),order:++order}));}
    }
    return out;
  }
  function previewSchedule(input={}){
    const categoryId=ClassicRCEFRARules.categories[input.category]?input.category:(event?.category||'10-offroad'),c=ClassicRCEFRARules.categories[categoryId]||ClassicRCEFRARules.categories['10-offroad'];
    const settings={...(event?.settings||{}),...(input.settings||{})},pilotCount=Math.max(0,Number(input.pilotCount??event?.pilotIds?.length)||0),heatCount=Math.max(1,Math.ceil(Math.max(1,pilotCount)/ClassicRCEFRARules.maxDriversPerHeat));
    const date=input.date||event?.date||new Date().toISOString().slice(0,10),tl=CompetitionScheduler.make({date,startTime:settings.startTime||'09:00',meta:{mode:'classic-rc-preview',category:categoryId}});let cursor=tl.startEpoch,firstRound=true;
    const addBreak=(key,label,fallback,stage,meta={})=>{const durationMin=scheduleDuration(key,fallback,'break',settings);if(durationMin<=0)return;CompetitionScheduler.add(tl,{kind:'break',stage,label,durationMin,plannedStartEpoch:cursor,meta:{...meta,scheduleKey:key,preview:true}});cursor+=durationMin*60000;};
    const addHeat=(key,stage,label,round,heat,participantCount=pilotCount,group='')=>{const durationMin=scheduleDuration(key,c.raceMinutes,'heat',settings);CompetitionScheduler.add(tl,{kind:'heat',stage,label,subLabel:`${c.label} · ${participantCount} пилотов`,durationMin,minStartGapMin:c.minStartGapMin,plannedStartEpoch:cursor,participantCount,meta:{round,heat,group,scheduleKey:key,preview:true}});cursor+=Math.max(durationMin,c.minStartGapMin)*60000;};
    const beforeRound=(stage,round)=>{if(firstRound){firstRound=false;return;}addBreak(breakScheduleKey(stage,round),'ПАУЗА',Number(settings.roundBreakMin)||0,'break',{beforeStage:stage,beforeRound:round});};
    for(let r=1;r<=Math.max(1,Number(settings.seedingRounds)||2);r++){beforeRound('seeding',r);for(let h=1;h<=heatCount;h++)addHeat(heatScheduleKey('seeding',r,h),'seeding',`РАССТАНОВКА ${r} · ГРУППА ${h}/${heatCount}`,r,h,Math.min(10,Math.max(0,pilotCount-(h-1)*10)));}
    for(let r=1;r<=Math.max(1,Number(settings.controlledPracticeRounds)||2);r++){beforeRound('controlled',r);for(let h=1;h<=heatCount;h++)addHeat(heatScheduleKey('controlled',r,h),'controlled',`ПРАКТИКА ${r} · ГРУППА ${h}/${heatCount}`,r,h,Math.min(10,Math.max(0,pilotCount-(h-1)*10)));}
    for(let r=1;r<=Math.max(1,Number(settings.qualifyingRounds)||5);r++){beforeRound('qualifying',r);const seq=classicRCHeatOrder(categoryId,heatCount,r);for(const h of seq)addHeat(heatScheduleKey('qualifying',r,h),'qualifying',`КВ${r} · ГРУППА ${h}/${heatCount}`,r,h,Math.min(10,Math.max(0,pilotCount-(h-1)*10)));}
    addBreak(breakScheduleKey('pre-finals'),'ПЕРЕРЫВ ПЕРЕД ФИНАЛАМИ',Number(settings.finalBreakMin)||0,'pre-finals');
    const finalCount=Math.max(1,Math.ceil(Math.max(1,pilotCount)/10)),groups=Array.from({length:finalCount},(_,i)=>String.fromCharCode(65+i));
    if(settings.finalPractice!==false){const fp=categoryId==='10-offroad'?groups.filter(x=>x==='A'):groups;for(const name of [...fp].reverse())addHeat(heatScheduleKey('finalPractice',1,groups.indexOf(name)+1,name),'finalPractice',`ПРАКТИКА ФИНАЛ ${name}`,1,groups.indexOf(name)+1,0,name);addBreak(breakScheduleKey('final-series',1),'ПАУЗА ПЕРЕД ФИНАЛЬНЫМИ ЗАЕЗДАМИ',Number(settings.roundBreakMin)||0,'final-round-break',{finalSeries:1});}
    for(let leg=1;leg<=3;leg++){for(const name of [...groups].reverse())addHeat(heatScheduleKey('final',leg,groups.indexOf(name)+1,name),'final',`ФИНАЛ ${name} · ЗАЕЗД ${leg}/3`,leg,groups.indexOf(name)+1,0,name);if(leg<3)addBreak(breakScheduleKey('final-series',leg+1),`ПАУЗА МЕЖДУ ФИНАЛЬНЫМИ ЗАЕЗДАМИ · ${leg}/${leg+1}`,Number(settings.roundBreakMin)||0,'final-round-break',{finalSeries:leg+1});}
    const last=tl.items[tl.items.length-1],endEpoch=last?(Number(last.plannedStartEpoch)||tl.startEpoch)+(last.kind==='break'?Number(last.durationMin||0)*60000:Number(last.durationMin||c.raceMinutes)*60000):tl.startEpoch;
    return{timeline:tl,startEpoch:tl.startEpoch,endEpoch,totalMin:Math.max(0,(endEpoch-tl.startEpoch)/60000),heatCount,category:c};
  }
  function scheduleBuild(){
    ...
    event.events.forEach((ev,i)=>{
      if(i&&(ev.stage!==prevStage||ev.round!==prevRound)){const breakKey=breakScheduleKey(ev.stage,ev.round),breakMin=scheduleDuration(breakKey,roundBreak,'break');if(breakMin>0){CompetitionScheduler.add(tl,{kind:'break',stage:'break',label:'ПАУЗА',durationMin:breakMin,plannedStartEpoch:cursor,meta:{scheduleKey:breakKey}});cursor+=breakMin*60000;}}
      const heatKey=heatScheduleKey(ev.stage,ev.round,ev.heat,ev.groupName);ev.durationMin=scheduleDuration(heatKey,ev.durationMin||c.raceMinutes,'heat');CompetitionScheduler.add(tl,{kind:'heat',stage:ev.stage,label:ev.label,subLabel:`${c.label} · ${ev.pilots.length} пилотов`,eventKey:ev.key,durationMin:ev.durationMin,minStartGapMin:c.minStartGapMin,plannedStartEpoch:cursor,participantCount:ev.pilots.length,meta:{round:ev.round,heat:ev.heat,scheduleKey:heatKey}});
      cursor+=Math.max(ev.durationMin,c.minStartGapMin)*60000;prevStage=ev.stage;prevRound=ev.round;
    });
    const preFinalKey=breakScheduleKey('pre-finals'),preFinalMin=scheduleDuration(preFinalKey,Number(event.settings.finalBreakMin)||0,'break');if(preFinalMin>0){CompetitionScheduler.add(tl,{kind:'break',stage:'pre-finals',label:'ПЕРЕРЫВ ПЕРЕД ФИНАЛАМИ',durationMin:preFinalMin,plannedStartEpoch:cursor,meta:{scheduleKey:preFinalKey}});cursor+=preFinalMin*60000;}
    ...
    if(event.settings.finalPractice){const fp=event.category==='10-offroad'?predicted.filter(x=>x==='A'):predicted;for(const name of [...fp].reverse()){const heatKey=heatScheduleKey('finalPractice',1,predicted.indexOf(name)+1,name),durationMin=scheduleDuration(heatKey,c.raceMinutes,'heat');CompetitionScheduler.add(tl,{kind:'heat',stage:'finalPractice',label:`ПРАКТИКА ФИНАЛ ${name}`,subLabel:`${c.label} · состав после квалификации`,durationMin,minStartGapMin:c.minStartGapMin,plannedStartEpoch:cursor,participantCount:0,meta:{placeholder:true,group:name,leg:0,scheduleKey:heatKey}});cursor+=Math.max(durationMin,c.minStartGapMin)*60000;}const breakKey=breakScheduleKey('final-series',1),breakMin=scheduleDuration(breakKey,roundBreak,'break');if(breakMin>0){CompetitionScheduler.add(tl,{kind:'break',stage:'final-round-break',label:'ПАУЗА ПЕРЕД ФИНАЛЬНЫМИ ЗАЕЗДАМИ',durationMin:breakMin,plannedStartEpoch:cursor,meta:{finalSeries:1,scheduleKey:breakKey}});cursor+=breakMin*60000;}}
    for(let leg=1;leg<=3;leg++){for(const name of [...predicted].reverse()){const heatKey=heatScheduleKey('final',leg,predicted.indexOf(name)+1,name),durationMin=scheduleDuration(heatKey,c.raceMinutes,'heat');CompetitionScheduler.add(tl,{kind:'heat',stage:'final',label:`ФИНАЛ ${name} · ЗАЕЗД ${leg}/3`,subLabel:`${c.label} · состав после квалификации`,durationMin,minStartGapMin:c.minStartGapMin,plannedStartEpoch:cursor,participantCount:0,meta:{placeholder:true,group:name,leg,scheduleKey:heatKey}});cursor+=Math.max(durationMin,c.minStartGapMin)*60000;}if(leg<3){const breakKey=breakScheduleKey('final-series',leg+1),breakMin=scheduleDuration(breakKey,roundBreak,'break');if(breakMin>0){CompetitionScheduler.add(tl,{kind:'break',stage:'final-round-break',label:`ПАУЗА МЕЖДУ ФИНАЛЬНЫМИ ЗАЕЗДАМИ · ${leg}/${leg+1}`,durationMin:breakMin,plannedStartEpoch:cursor,meta:{finalSeries:leg+1,scheduleKey:breakKey}});cursor+=breakMin*60000;}}}
    event.timeline=tl;
  }
  ...
  function buildFinals(){if(event.finalGroups.length)return;event.finalGroups=classicRCFinalGroupsFromQualification(event.qualifyingStandings,10,event.settings.lowestFinalPolicy||'keep');const c=category();let order=Math.max(0,...event.events.map(e=>e.order||0));
    ...
    if(event.settings.finalPractice){const groups=event.category==='10-offroad'?event.finalGroups.filter(g=>g.name==='A'):event.finalGroups;for(const g of [...groups].reverse()){const heatIndex=event.finalGroups.indexOf(g)+1;event.events.push(makeHeat({stage:'finalPractice',round:1,heat:heatIndex,groupName:g.name,pilots:g.pilots,label:`ПРАКТИКА ФИНАЛ ${g.name}`,durationMin:scheduleDuration(heatScheduleKey('finalPractice',1,heatIndex,g.name),c.raceMinutes,'heat'),order:++order}));}}
    for(let leg=1;leg<=3;leg++)for(const g of [...event.finalGroups].reverse()){const heatIndex=event.finalGroups.indexOf(g)+1;event.events.push(makeHeat({stage:'final',round:leg,heat:heatIndex,groupName:g.name,pilots:g.pilots,label:`ФИНАЛ ${g.name} · ЗАЕЗД ${leg}/3`,durationMin:scheduleDuration(heatScheduleKey('final',leg,heatIndex,g.name),c.raceMinutes,'heat'),order:++order}));}
    /* Bind pre-built neutral schedule placeholders to the real finals. */
    event.events.filter(e=>['finalPractice','final'].includes(e.stage)).forEach(ev=>{const item=event.timeline?.items?.find(x=>x.kind==='heat'&&!x.eventKey&&x.stage===ev.stage&&String(x.meta?.group||'')===String(ev.groupName||'')&&Number(x.meta?.leg||0)===Number(ev.stage==='final'?ev.round:0));if(item){ev.durationMin=Number(item.durationMin)||ev.durationMin;item.eventKey=ev.key;item.participantCount=ev.pilots.length;item.subLabel=`${c.label} · ${ev.pilots.length} пилотов`;item.meta.placeholder=false;}else{const cursor=event.timeline?.items?.length?Math.max(...event.timeline.items.map(i=>i.plannedStartEpoch+(i.kind==='break'?i.durationMin:Math.max(i.durationMin,c.minStartGapMin))*60000)):Date.now();CompetitionScheduler.add(event.timeline,{kind:'heat',stage:ev.stage,label:ev.label,subLabel:`${c.label} · ${ev.pilots.length} пилотов`,eventKey:ev.key,durationMin:ev.durationMin,minStartGapMin:c.minStartGapMin,plannedStartEpoch:cursor,participantCount:ev.pilots.length,meta:{group:ev.groupName,leg:ev.round,scheduleKey:heatScheduleKey(ev.stage,ev.round,ev.heat,ev.groupName)}});}});
