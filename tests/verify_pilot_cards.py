...
checks['models_are_tap_toggle']='data-race-model-toggle' in pilot and 'pilotToggleRaceModel' in pilot
checks['rc24_square_avatar']='aspect-ratio:1/1' in css and '.pilotTilePortrait' in css
checks['rc24_avatar_no_inner_frame']=re.search(r'\.pilotTilePortrait\{[^}]*border:0[^}]*border-radius:0',css) is not None
checks['rc24_flag_lower_right_flat']=re.search(r'\.pilotTileFlag\{[^}]*right:0[^}]*bottom:0[^}]*border:0!important[^}]*border-radius:0!important[^}]*box-shadow:none!important',css) is not None
checks['rc24_flat_model_strips']=re.search(r'\.pilotModelTile\{[^}]*border:0[^}]*border-radius:0',css) is not None and 'height:38px' in css
checks['rc24_no_colored_selection_outline']='border-color:var(--pilot-model-color)' not in css and 'transform:translate' not in re.search(r'\.pilotModelTile\.selectable.*?(?=\n\n/\* Selected participants)',css,re.S).group(0)
checks['rc24_editor_flat_fields']=re.search(r'\.pilotEditorIdentity input.*?border:0;border-bottom:',css,re.S) is not None and 'border-radius:0' in re.search(r'\.pilotEditorIdentity input.*?\}',css,re.S).group(0)
checks['rc24_compact_race_tiles']='pilotRaceSetupPortrait' in pilot and 'pilotRaceSetupCaption' in pilot and 'pilotCompactName' in pilot and 'pilotRaceSetupGrid' in views
checks['rc24_race_tile_is_not_old_row']='pilotRaceSetupPilot' not in pilot and 'raceSetupModel' not in pilot
checks['rc24_editor_flag_tracks_country']='pilotEditorAvatarMarkup' in pilot and "$('#mCountry').addEventListener('change'" in pilot
