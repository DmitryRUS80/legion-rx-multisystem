...
- Настройки больше не открывают раздел «Общие» автоматически. При входе показывается список разделов; содержимое появляется только после выбора.
- Раздел «Обновление» получил живую строку состояния скачивания и прогресс-бар по фактически закэшированным файлам нового пакета.
- Кнопка «Установить обновление» появляется только после полного скачивания и проверки candidate package.
- После установки и обязательной перезагрузки приложение возвращается в «Настройки → Обновление», а не в «Общие» и не на другой экран.
- Архитектура, RallyCross, LapWiz, session control и pilot profile link не менялись.
