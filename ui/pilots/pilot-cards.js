...
function pilotModal(existing=null,addToRace=false,originRect=null){
  ...
  $('#savePilotModal').onclick=()=>{
    ...
    const racePilot=pilotRaceEntryForProfile(id),raceModel=racePilot?(models.find(m=>String(m.id)===String(racePilot.modelId))||first):first;
    if(racePilot){
      const nextTransponder=String(raceModel?.transponder||raceModel?.number||profile.transponder||'').trim();
      const conflict=(state.race?.pilots||[]).find(p=>String(p.id)!==String(racePilot.id)&&String(p.transponder||'')===nextTransponder);
      if(!nextTransponder)return toast('Введите ID транспондера для модели, выбранной в соревновании');
      if(conflict)return toast(`Транспондер ${nextTransponder} уже используется: ${conflict.name}`);
    }
    ...
    if(racePilot){
      const nextTransponder=String(raceModel?.transponder||raceModel?.number||profile.transponder||'').trim();
      const res=updateActiveRacePilotIdentity(racePilot.id,{name,transponder:nextTransponder,country:profile.country,club:profile.club});
