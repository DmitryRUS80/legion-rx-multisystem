...
function uiIcon(name,cls='uiIcon'){
 ...
 return `<svg class="${cls}" data-rx-icon="${esc(name)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name]||paths.flag}</svg>`;
}
...
function applySettings(){document.documentElement.dataset.theme=state.settings.theme;document.documentElement.dataset.skin=state.settings.uiSkin||'classic';document.documentElement.lang=state.settings.lang;const themeMeta=document.querySelector('meta[name="theme-color"]');if(themeMeta)themeMeta.content=(state.settings.uiSkin==='light'||(state.settings.uiSkin==='classic'&&state.settings.theme==='light'))?'#f4f7fa':'#0b1118';lapwiz.sound=state.settings.lapSound;applyUiTokens();applyAppBackground();translateStatic();announcer.enabled=state.settings.announcerEnabled;announcer.startMode=state.settings.startVoiceMode;if(window.RXUISkins)window.RXUISkins.applyIcons();}
...
function raceSvg(name,cls='raceSvg'){
 ...
 return `<svg class="${cls}" data-rx-icon="${esc(name)}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.15" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name]||paths.flag}</svg>`;
}
