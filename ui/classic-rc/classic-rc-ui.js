...
function classicRCHeatProtocolsSection(c){const completed=(c?.events||[]).filter(e=>e.saved&&!e.cancelled);if(!completed.length)return'';const blocks=completed.map(ev=>{const eventBest=Math.min(...(ev.pilots||[]).map(id=>Number(ev.lapStats?.[id]?.bestLapMs)).filter(x=>Number.isFinite(x)&&x>0),Infinity);const resultById=new Map((ev.result||[]).map((r,i)=>[String(r.pilotId),{...r,_index:i}]));const pilotCards=(ev.pilots||[]).map(id=>{const pilot=ClassicRCEngine.pilot(id),r=resultById.get(String(id))||{},st=ev.lapStats?.[id]||{},place=r.place??(Number.isFinite(r._index)?r._index+1:'—'),status=String(r.status||'FIN'),best=Number(st.bestLapMs),isEventBest=Number.isFinite(best)&&best===eventBest;return `<article class="classicHeatPilotCard"><div class="classicHeatPilotHead"><strong>${status==='FIN'?place:esc(status)}</strong><b>${esc(rxnPilotDisplayName(pilot))}</b></div><div class="classicHeatMetrics"><span><small>КРУГИ</small><b>${Number(r.laps??st.laps)||0}</b></span><span><small>РЕЗУЛЬТАТ</small><b>${Number(r.timeMs)>0?rxnFormatDuration(r.timeMs):'—'}</b></span><span class="${isEventBest?'good':''}"><small>BEST LAP</small><b>${Number.isFinite(best)&&best>0?rxnFormatDuration(best):'—'}</b></span></div>${classicRCLapChips(c,ev,id)}</article>`;}).join('');return `<details class="classicHeatProtocol"><summary><span>${esc(ClassicRCEngine.stageLabel(ev.stage))}</span><b>${esc(ev.label)}</b><em>${(ev.pilots||[]).length} PILOTS</em></summary><div class="classicHeatProtocolBody">${pilotCards}</div></details>`;}).join('');return `<section class="classicResultsSection classicHeatProtocols"><div class="classicResultsSectionHead"><h3>ПРОТОКОЛЫ ЗАЕЗДОВ</h3><p>Открой нужный заезд: круги, итоговое время, лучший круг и полная раскладка кругов. В seeding зелёным выделена лучшая связка.</p></div>${blocks}</section>`;}
