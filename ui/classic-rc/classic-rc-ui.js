...
function classicRCSeedingSection(c,seed){
  const rows=seed.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId),window=classicRCSeedWindow(c,r.pilotId),range=window?`${window.startIndex}–${window.endIndex}`:'—';return `<tr><td class="classicProtocolPos">${r.rank}</td><td class="classicProtocolPilot"><b>${esc(rxnPilotDisplayName(pilot))}</b></td><td class="classicProtocolGood"><b>${classicRCSeedBestMain(r.best)}</b></td><td>${esc(range)}</td></tr>`;}).join('');
  return `<section class="classicResultsSection"><div class="classicProtocolTitle"><h3>SEEDING</h3><span>BEST ${c.settings.seedingConsecutiveLaps} AVG</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicProtocolStandings"><thead><tr><th>P</th><th>ПИЛОТ</th><th>BEST ${c.settings.seedingConsecutiveLaps} AVG</th><th>КРУГИ</th></tr></thead><tbody>${rows||'<tr><td colspan="4">НЕТ ДАННЫХ</td></tr>'}</tbody></table></div></section>`;
}
function classicRCQualifyingSection(c,q,rounds){
  const rows=q.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);return `<tr><td class="classicProtocolPos">${r.rank}</td><td class="classicProtocolPilot"><b>${esc(rxnPilotDisplayName(pilot))}</b></td>${Array.from({length:rounds},(_,i)=>{const rr=r.rounds.find(x=>x.round===i+1),counted=r.counting.some(x=>x.round===i+1);return `<td class="${counted?'classicProtocolGood':''}">${rr?rr.points:'—'}</td>`;}).join('')}<td class="classicProtocolGood"><b>${r.valid?r.total:'—'}</b></td></tr>`;}).join('');
  return `<section class="classicResultsSection"><div class="classicProtocolTitle"><h3>QUALIFYING</h3><span>ROUND-BY-ROUND · В ЗАЧЁТ ${classicRCQualifyingRoundsToCount(Number(c.settings.qualifyingRounds||5))}</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicProtocolStandings"><thead><tr><th>P</th><th>ПИЛОТ</th>${Array.from({length:rounds},(_,i)=>`<th>Q${i+1}</th>`).join('')}<th>COUNT</th></tr></thead><tbody>${rows||'<tr><td colspan="8">НЕТ ДАННЫХ</td></tr>'}</tbody></table></div></section>`;
}
function classicRCFinalsSection(c,finals){
  return [...finals].sort((a,b)=>a.groupName.localeCompare(b.groupName)).map(g=>{const rows=g.standings.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId),counted=new Set((r.counting||[]).map(x=>Number(x.leg)));return `<tr><td class="classicProtocolPos">${r.rank}</td><td class="classicProtocolPilot"><b>${esc(rxnPilotDisplayName(pilot))}</b></td>${[1,2,3].map(n=>{const x=r.all.find(y=>y.leg===n);return `<td class="${counted.has(n)?'classicProtocolGood':''}">${x?x.points:'—'}</td>`;}).join('')}<td class="classicProtocolGood"><b>${r.counting.length>=2?r.total:'—'}</b></td></tr>`;}).join('');return `<section class="classicResultsSection"><div class="classicProtocolTitle"><h3>FINAL ${esc(g.groupName)}</h3><span>3 LEGS · BEST 2</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicProtocolStandings"><thead><tr><th>P</th><th>ПИЛОТ</th><th>LEG 1</th><th>LEG 2</th><th>LEG 3</th><th>BEST 2</th></tr></thead><tbody>${rows||'<tr><td colspan="6">НЕТ ДАННЫХ</td></tr>'}</tbody></table></div></section>`;}).join('');
}
function classicRCOverallProtocolSection(c){
  const rows=c?.finalProtocol||[],body=rows.map(r=>`<tr><td class="classicProtocolPos">${r.overallRank}</td><td class="classicProtocolPilot"><b>${esc(rxnPilotDisplayName(ClassicRCEngine.pilot(r.pilotId)))}</b></td><td>${esc(r.groupName)}</td><td class="classicProtocolGood"><b>${r.total}</b></td></tr>`).join('');
  return `<section class="classicResultsSection"><div class="classicProtocolTitle"><h3>ИТОГОВЫЙ ПРОТОКОЛ</h3><span>CLASSIC RC · EFRA</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicProtocolStandings"><thead><tr><th>P</th><th>ПИЛОТ</th><th>FINAL</th><th>BEST 2</th></tr></thead><tbody>${body||'<tr><td colspan="4">ИТОГОВЫЙ ПРОТОКОЛ ЕЩЁ НЕ СФОРМИРОВАН</td></tr>'}</tbody></table></div></section>`;
}
...
function classicRCHeatProtocolsSection(c){
  ...
  const blocks=completed.map(ev=>{
    ...
    const rows=ordered.map(id=>{const pilot=ClassicRCEngine.pilot(id),r=resultById.get(String(id))||{},st=ev.lapStats?.[id]||{},status=String(r.status||'FIN'),place=status==='FIN'?(r.place??(Number.isFinite(r._index)?r._index+1:'—')):status,best=Number(st.bestLapMs),isEventBest=Number.isFinite(best)&&best===eventBest,points=classicRCEventPoints(c,ev,id),seed=hasSeed?classicRCSeedWindow(c,id):null,note=seed&&seed.eventKey===ev.key?`BEST ${Number(c.settings.seedingConsecutiveLaps)||3}: ${rxnFormatDuration(seed.avgMs)} · L${seed.startIndex}–${seed.endIndex}`:'';return `<tr><td class="classicProtocolPos">${esc(place)}</td><td class="classicProtocolPilot"><b>${esc(rxnPilotDisplayName(pilot))}</b></td><td>${Number(r.laps??st.laps)||0}</td><td>${Number(r.timeMs)>0?rxnFormatDuration(r.timeMs):'—'}</td><td class="${isEventBest?'classicProtocolGood':''}">${Number.isFinite(best)&&best>0?rxnFormatDuration(best):'—'}</td>${hasPoints?`<td>${points}</td>`:''}<td>${esc(note||'—')}</td></tr>`;}).join('');
    const stageLabel=ClassicRCEngine.stageLabel(ev.stage),summaryCols=hasPoints?7:6;
    return `<details class="classicHeatProtocol"><summary><span>${esc(stageLabel)}</span><b>${esc(ev.label)}</b><em>${(ev.pilots||[]).length} PILOTS</em></summary><div class="classicHeatProtocolBody"><div class="classicProtocolSubhead"><h4>РЕЗУЛЬТАТ ЗАЕЗДА</h4><span>${esc(ev.label)}</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicHeatOverview"><thead><tr><th>P</th><th>ПИЛОТ</th><th>К</th><th>ИТОГО</th><th>ЛУЧШАЯ</th>${hasPoints?'<th>ОЧКИ</th>':''}<th>ЗАМЕТКА</th></tr></thead><tbody>${rows||`<tr><td colspan="${summaryCols}">НЕТ РЕЗУЛЬТАТА</td></tr>`}</tbody></table></div><div class="classicProtocolSubhead classicLapSubhead"><h4>СПИСОК КРУГОВ</h4><span>Время кругов по каждому пилоту</span></div>${classicRCLapMatrix(c,ev,ordered)}</div></details>`;
  }).join('');
