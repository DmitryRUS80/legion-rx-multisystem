...
function classicRCTimerPanel(){
  ...
  const lastLap=['qualifying','final'].includes(ev?.stage)?' + LAST LAP':'',mode=ev?.startMode==='staggered'?` · ПО ОДНОМУ · ${(Number(c?.settings?.staggerIntervalMs)||2000)/1000} СЕК`:'';
...
function classicRCParticipantCard(id,{withdrawn=false,candidate=false,locked=false}={}){
  const p=classicRCParticipantProfile(id),models=typeof pilotModels==='function'?pilotModels(p):[],model=models[0]||null,idText=String(model?.transponder||model?.number||p?.transponder||'—'),className=String(model?.className||'').trim(),color=String(model?.uiColor||pilotStableColor?.(p?.id||idText)||'#999'),action=candidate?'late-add':'late-remove',label=candidate?(withdrawn?'ВЕРНУТЬ':'ДОБАВИТЬ'):'УДАЛИТЬ';
  const team=typeof pilotTeamBadge==='function'?pilotTeamBadge(p):'',flag=typeof pilotFlagBadge==='function'?pilotFlagBadge(p):'',avatar=typeof pilotAvatarMarkup==='function'?pilotAvatarMarkup(p,'classicParticipantPhoto'):'';
  return `<article class="classicParticipantCard ${withdrawn?'withdrawn':''}"><div class="classicParticipantPortrait">${avatar}${team}${flag}</div><div class="classicParticipantMain"><b>${esc(typeof pilotCompactName==='function'?pilotCompactName(p?.name||'ПИЛОТ'):rxnPilotDisplayName(p))}</b><div class="classicParticipantModel" style="--pilot-model-color:${esc(color)}"><strong>${esc(idText)}</strong><span>${esc((className||model?.name||'МОДЕЛЬ').toUpperCase())}</span></div>${withdrawn?'<em>НЕ УЧАСТВУЕТ В БУДУЩИХ ЗАЕЗДАХ</em>':''}</div><button type="button" class="${candidate?'add':'remove'}" data-classic-action="${action}" data-pilot-id="${esc(id)}" ${locked?'disabled':''}>${label}</button></article>`;
}
function classicRCParticipantsDrawer(){
  ...
  return `${tail}<div class="classicParticipantsScrim" data-classic-action="participants-close"></div><aside class="classicParticipantsDrawer"><header><div><small>EFRA RC · ДИРЕКТОР</small><h2>УЧАСТНИКИ</h2><p>Изменения действуют только для ещё не начавшихся заездов.</p></div><button type="button" data-classic-action="participants-close">×</button></header><div class="classicParticipantsBody"><section><div class="classicParticipantsSectionHead"><b>В СОРЕВНОВАНИИ</b><span>${active.length}</span></div><div class="classicParticipantsList ${active.length>10?'twoCol':''}">${active.map(id=>classicRCParticipantCard(id,{locked})).join('')||'<div class="rxnEmpty">Нет активных участников.</div>'}</div></section><section><div class="classicParticipantsSectionHead"><b>ДОБАВИТЬ ИЗ БАЗЫ</b><span>${locked?'ФИНАЛЫ СФОРМИРОВАНЫ':candidates.length}</span></div>${locked?'<div class="classicParticipantsLocked">После формирования финалов состав защищён. Нового пилота можно добавить только в следующее соревнование.</div>':`<div class="classicParticipantsList ${candidates.length>10?'twoCol':''}">${candidates.map(p=>classicRCParticipantCard(p.id,{candidate:true,withdrawn:withdrawn.has(String(p.id))})).join('')||'<div class="rxnEmpty">Все пилоты базы уже участвуют.</div>'}</div>`}</section></div><footer><button type="button" class="classicScheduleAction primary" data-classic-action="late-new" ${locked?'disabled':''}>+ НОВЫЙ ПИЛОТ</button><button type="button" class="classicScheduleAction" data-classic-action="participants-close">ЗАКРЫТЬ</button></footer></aside>`;
}
...
function classicRCStaggerPanel(){
  ...
  return `<aside class="classicStaggerPanel" data-classic-stagger-panel><small>СТАРТ ПО ОДНОМУ · ${((Number(ClassicRCEngine.get()?.settings?.staggerIntervalMs)||2000)/1000).toFixed(1)} СЕК</small><div><span>СТАРТУЕТ</span><b data-classic-stagger-current>${current?esc(rxnPilotDisplayName(current)):'—'}</b><em data-classic-stagger-progress>${idx}/${total}</em></div><div class="next"><span>ДАЛЕЕ</span><b data-classic-stagger-next>${next&&s.staggerNextPilotId?esc(rxnPilotDisplayName(next)):'—'}</b></div></aside>`;
}
...
function classicRCSaveRaceSettings(){
  ...
  if(c&&ev&&!durationLocked){const res=ClassicRCEngine.updateCurrentHeatSettings({durationMin:Number($('#classicRaceDuration')?.value),minLapSec:Number($('#classicRaceMinLap')?.value),staggerIntervalMs:Math.round((Number($('#classicStaggerInterval')?.value)||2)*1000)});if(!res?.ok)return toast(res?.error||'Не удалось изменить заезд');}
  if(c&&(!ev||durationLocked)){c.settings.staggerIntervalMs=Math.max(500,Math.min(5000,Math.round((Number($('#classicStaggerInterval')?.value)||2)*1000)));ClassicRCEngine.persist();}state.settings.announcerEnabled=Boolean($('#classicAnnouncerToggle')?.checked);state.settings.lapSound=Boolean($('#classicLapSoundToggle')?.checked);state.settings.voiceStartCall=Boolean($('#classicVoiceStartToggle')?.checked);state.settings.voiceBestLap=Boolean($('#classicVoiceBestToggle')?.checked);state.settings.voiceFinish=Boolean($('#classicVoiceFinishToggle')?.checked);state.settings.voiceService=Boolean($('#classicVoiceServiceToggle')?.checked);state.settings.voiceResults=Boolean($('#classicVoiceResultsToggle')?.checked);state.settings.pilotVoiceEnabled=Boolean($('#classicPilotVoiceToggle')?.checked);const auto=CompetitionAutoPilot.ensure(c);auto.autoSave=Boolean($('#classicAutoSaveToggle')?.checked);auto.reviewDelayMs=Math.max(0,Math.min(60000,(Number($('#classicAutoReviewSec')?.value)||0)*1000));ClassicRCEngine.persist();save(KEYS.settings,state.settings);applySettings();closeModal();toast('Настройки EFRA RC сохранены');render();
}
...
async function classicRCDispatchAction(b){
  ...
  if(a==='late-remove'){const p=ClassicRCEngine.pilot(b.dataset.pilotId);if(!confirm(`Удалить ${rxnPilotDisplayName(p)} из будущих заездов? Уже полученные результаты сохранятся.`))return;const r=ClassicRCEngine.withdrawPilot(b.dataset.pilotId,ClassicRCRuntime.nowEpoch());if(!r?.ok)return toast(r?.error||'Не удалось убрать пилота');toast('Пилот удалён из будущих заездов');classicParticipantsOpen=true;classicRCSyncScheduleHost(false);return;}
