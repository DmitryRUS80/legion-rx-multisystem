...
function classicRCScheduleFooter(snap){
  const c=ClassicRCEngine.get(),s=ClassicRCRuntime.session(),current=snap.current,nextHeat=CompetitionScheduler.nextHeat(c?.timeline),hold=Boolean(c?.directorHold?.active),tl=c?.timeline;
...
function classicRCSyncScheduleHost(preserveScroll=true){
  ...
  const oldList=preserveScroll?host.querySelector('.classicScheduleList'):null,scrollTop=oldList?.scrollTop||0;
...
function classicRCCockpitView(){rxnEnsureSystemClockTicker();classicRCEnsureScheduleTicker();const c=ClassicRCEngine.get();if(!c||c.status==='setup')return `<section class="page"><div class="card"><h2>Classic RC ещё не подготовлен</h2><button class="btn primary" data-classic-action="setup">К НАСТРОЙКЕ</button></div></section>`;const ev=ClassicRCRuntime.event(),cls=rxnColumnClass(),count=rxnMetricCount();if(!ev&&c.status==='finished')return `<section class="rxnCockpit classicRCCockpit classicResultsCockpit ${cls}" style="--rxn-metric-count:${count}">${classicRCHeader()}${classicRCTitle()}<main class="classicResultsMain"><section class="classicResultsRoster"><div class="classicFinishedActions"><button class="btn primary" data-classic-action="results">ВСЕ РЕЗУЛЬТАТЫ</button><button class="btn secondary" data-classic-action="schedule">РАСПИСАНИЕ</button><button class="btn secondary" data-classic-action="setup">СОБЫТИЕ</button><button class="btn secondary" data-classic-action="home">ГЛАВНАЯ</button><button class="btn danger" data-classic-action="archive">ЗАВЕРШИТЬ</button></div><div class="classicFinalProtocol">${classicRCFinalProtocolMarkup()}</div></section></main></section>`;ClassicRCRuntime.ensureSession();return `<section class="rxnCockpit classicRCCockpit ${cls}" style="--rxn-metric-count:${count}">${classicRCHeader()}${classicRCTitle()}<main class="rxnMain"><section class="rxnRoster"><div id="classicPilotBoard" class="rxnTable">${classicRCPilotTable()}</div></section><aside class="rxnSide">${classicRCScheduleStatusStrip(true)}${classicRCTimerPanel()}${classicRCMobileInfo()}${classicRCControls()}</aside></main></section>`;}
...
function classicRCTieNote(row,allRows,type){
  if(!row||!Array.isArray(allRows))return'';
  const tied=allRows.some(x=>x!==row&&Number(x.total)===Number(row.total));
  if(!tied||!row.counting?.length)return'';
  const parts=[...row.counting].sort((a,b)=>Number(a.points)-Number(b.points)).map(x=>`${x.laps||0}L / ${rxnFormatDuration(x.timeMs||0)}`);
  return `Тай-брейк: ${parts.join(' · ')}`;
}
...
function classicRCSeedBestDetail(c,pilotId,best){if(!best)return'Нет связки';const view=classicRCSeedWindow(c,pilotId),range=view?`КРУГИ ${view.startIndex}–${view.endIndex}`:`${best.laps?.length||0} КРУГА`,laps=(view?.laps||best.laps||[]).map(x=>rxnFormatDuration(x)).join(' · ');return `${range}${laps?` · ${laps}`:''}`;}
function classicRCRoundPointText(round){if(!round)return'—';const base=`Q${round.round}: ${round.points}`;if(Number(round.laps)>0&&Number.isFinite(Number(round.timeMs)))return `${base} · ${round.laps}L / ${rxnFormatDuration(round.timeMs)}`;return base;}
function classicRCFinalLegText(leg){if(!leg)return'—';const base=`A${leg.leg}: ${leg.points}`;if(Number(leg.laps)>0&&Number.isFinite(Number(leg.timeMs)))return `${base} · ${leg.laps}L / ${rxnFormatDuration(leg.timeMs)}`;return base;}
function classicRCResultsOverview(c){const seedDone=(c.events||[]).filter(e=>e.stage==='seeding'&&e.saved).length,seedTotal=(c.events||[]).filter(e=>e.stage==='seeding').length,qDone=(c.events||[]).filter(e=>e.stage==='qualifying'&&e.saved).length,qTotal=(c.events||[]).filter(e=>e.stage==='qualifying').length,finalDone=(c.events||[]).filter(e=>e.stage==='final'&&e.saved).length,finalTotal=(c.events||[]).filter(e=>e.stage==='final').length,pilots=(c.pilotIds||[]).length;return `<section class="classicResultsSummaryGrid"><article><small>ПИЛОТЫ</small><b>${pilots}</b><span>${esc(ClassicRCEngine.category().label)}</span></article><article><small>SEEDING</small><b>${seedDone}/${seedTotal}</b><span>BEST ${c.settings.seedingConsecutiveLaps} AVG</span></article><article><small>QUALIFYING</small><b>${qDone}/${qTotal}</b><span>В ЗАЧЁТ: ${classicRCQualifyingRoundsToCount(Number(c.settings.qualifyingRounds||5))}</span></article><article><small>FINALS</small><b>${finalDone}/${finalTotal}</b><span>3 LEGS · BEST 2</span></article></section>`;}
function classicRCSeedingSection(c,seed){const cols=3;const rows=seed.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId),main=classicRCSeedBestMain(r.best),detail=classicRCSeedBestDetail(c,r.pilotId,r.best);return `<tr><td>${r.rank}</td><td><b>${esc(rxnPilotDisplayName(pilot))}</b></td><td class="counted"><b>${main}</b><div class="classicCellNote">${esc(detail)}</div></td></tr>`;}).join('');const cards=seed.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);return `<article class="classicResultCard"><div class="classicResultCardTop"><strong>${r.rank}</strong><b>${esc(rxnPilotDisplayName(pilot))}</b><em>${classicRCSeedBestMain(r.best)}</em></div><div class="classicResultCardMeta"><span class="classicResultBadge good">BEST ${c.settings.seedingConsecutiveLaps} AVG</span><span>${esc(classicRCSeedBestDetail(c,r.pilotId,r.best))}</span></div></article>`;}).join('');return `<section class="classicResultsSection"><div class="classicResultsSectionHead"><h3>SEEDING</h3><p>Стартовая расстановка строится по лучшей связке из ${c.settings.seedingConsecutiveLaps} подряд идущих кругов. Зелёным показан результат, который реально идёт в расчёт.</p></div><div class="classicResultsDesktop"><div class="tableWrap"><table class="table classicResultsTable"><thead><tr><th>POS</th><th>ПИЛОТ</th><th>BEST ${c.settings.seedingConsecutiveLaps} AVG</th></tr></thead><tbody>${rows||`<tr><td colspan="${cols}">—</td></tr>`}</tbody></table></div></div><div class="classicResultsMobile">${cards||'<div class="rxnEmpty">Нет данных seeding.</div>'}</div></section>`;}
function classicRCQualifyingSection(c,q,rounds){const cols=3+rounds;const rows=q.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);const countedText=r.counting?.length?r.counting.map(classicRCRoundPointText).join(' · '):'Нет зачётных раундов',tie=classicRCTieNote(r,q,'q');return `<tr><td>${r.rank}</td><td><b>${esc(rxnPilotDisplayName(pilot))}</b></td>${Array.from({length:rounds},(_,i)=>{const rr=r.rounds.find(x=>x.round===i+1),counted=r.counting.some(x=>x.round===i+1);return `<td class="${counted?'counted':''}">${rr?rr.points:'—'}</td>`;}).join('')}<td class="counted"><b>${r.valid?r.total:'—'}</b><div class="classicCellNote">${esc(countedText)}</div>${tie?`<div class="classicTieInfo">${esc(tie)}</div>`:''}</td></tr>`;}).join('');const cards=q.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);const roundChips=Array.from({length:rounds},(_,i)=>{const rr=r.rounds.find(x=>x.round===i+1),counted=r.counting.some(x=>x.round===i+1);return `<span class="classicResultBadge ${counted?'good':''}">Q${i+1} ${rr?rr.points:'—'}</span>`;}).join('');const countedText=r.counting?.length?r.counting.map(classicRCRoundPointText).join(' · '):'Нет зачётных раундов',tie=classicRCTieNote(r,q,'q');return `<article class="classicResultCard"><div class="classicResultCardTop"><strong>${r.rank}</strong><b>${esc(rxnPilotDisplayName(pilot))}</b><em>${r.valid?r.total:'—'}</em></div><div class="classicResultCardMeta"><span class="classicResultBadge good">COUNT ${r.valid?r.total:'—'}</span><span>${esc(countedText)}</span>${tie?`<span class="classicTieInfo">${esc(tie)}</span>`:''}</div><div class="classicResultChips">${roundChips}</div></article>`;}).join('');return `<section class="classicResultsSection"><div class="classicResultsSectionHead"><h3>QUALIFYING</h3><p>Round-by-Round. Очки: 0, 2, 3, 4… Чем меньше сумма лучших зачётных раундов, тем выше место.</p></div><div class="classicResultsDesktop"><div class="tableWrap"><table class="table classicResultsTable classicQualifyingTable"><thead><tr><th>POS</th><th>ПИЛОТ</th>${Array.from({length:rounds},(_,i)=>`<th>Q${i+1}</th>`).join('')}<th>COUNT</th></tr></thead><tbody>${rows||`<tr><td colspan="${cols}">—</td></tr>`}</tbody></table></div></div><div class="classicResultsMobile">${cards||'<div class="rxnEmpty">Нет квалификационных данных.</div>'}</div></section>`;}
function classicRCFinalsSection(c,finals){return finals.sort((a,b)=>a.groupName.localeCompare(b.groupName)).map(g=>{const rows=g.standings.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);const counted=new Set((r.counting||[]).map(x=>Number(x.leg)));const countedText=r.counting?.length?r.counting.map(classicRCFinalLegText).join(' · '):'Нет двух зачётных финалов',tie=classicRCTieNote(r,g.standings,'final');return `<tr><td>${r.rank}</td><td><b>${esc(rxnPilotDisplayName(pilot))}</b></td>${[1,2,3].map(n=>{const x=r.all.find(y=>y.leg===n);return `<td class="${counted.has(n)?'counted':''}">${x?x.points:'—'}</td>`;}).join('')}<td class="counted"><b>${r.counting.length>=2?r.total:'—'}</b><div class="classicCellNote">${esc(countedText)}</div>${tie?`<div class="classicTieInfo">${esc(tie)}</div>`:''}</td></tr>`;}).join('');const cards=g.standings.map(r=>{const pilot=ClassicRCEngine.pilot(r.pilotId);const counted=new Set((r.counting||[]).map(x=>Number(x.leg)));const legChips=[1,2,3].map(n=>{const x=r.all.find(y=>y.leg===n);return `<span class="classicResultBadge ${counted.has(n)?'good':''}">A${n} ${x?x.points:'—'}</span>`;}).join('');const countedText=r.counting?.length?r.counting.map(classicRCFinalLegText).join(' · '):'Нет двух зачётных финалов',tie=classicRCTieNote(r,g.standings,'final');return `<article class="classicResultCard"><div class="classicResultCardTop"><strong>${r.rank}</strong><b>${esc(rxnPilotDisplayName(pilot))}</b><em>${r.counting.length>=2?r.total:'—'}</em></div><div class="classicResultCardMeta"><span class="classicResultBadge good">BEST 2</span><span>${esc(countedText)}</span>${tie?`<span class="classicTieInfo">${esc(tie)}</span>`:''}</div><div class="classicResultChips">${legChips}</div></article>`;}).join('');return `<section class="classicResultsSection"><div class="classicResultsSectionHead"><h3>FINAL ${esc(g.groupName)}</h3><p>Официальная логика не менялась: 3 финала, в зачёт идут 2 лучших результата.</p></div><div class="classicResultsDesktop"><div class="tableWrap"><table class="table classicResultsTable classicFinalsTable"><thead><tr><th>POS</th><th>ПИЛОТ</th><th>LEG 1</th><th>LEG 2</th><th>LEG 3</th><th>BEST 2</th></tr></thead><tbody>${rows||'<tr><td colspan="6">—</td></tr>'}</tbody></table></div></div><div class="classicResultsMobile">${cards||'<div class="rxnEmpty">Нет финальных данных.</div>'}</div></section>`;}).join('');}
function classicRCOverallProtocolSection(c){const rows=c?.finalProtocol||[];const desktopRows=rows.map(r=>`<tr><td>${r.overallRank}</td><td><b>${esc(rxnPilotDisplayName(ClassicRCEngine.pilot(r.pilotId)))}</b></td><td><span class="classicResultBadge good">FINAL ${esc(r.groupName)}</span></td><td class="counted"><b>${r.total}</b></td></tr>`).join('');const cards=rows.map(r=>`<article class="classicResultCard"><div class="classicResultCardTop"><strong>${r.overallRank}</strong><b>${esc(rxnPilotDisplayName(ClassicRCEngine.pilot(r.pilotId)))}</b><em>${r.total}</em></div><div class="classicResultCardMeta"><span class="classicResultBadge good">${esc(r.groupName)} FINAL</span><span>BEST 2 = ${r.total}</span></div></article>`).join('');return `<section class="classicResultsSection"><div class="classicResultsSectionHead"><h3>ИТОГОВЫЙ ПРОТОКОЛ</h3><p>Это экран удобного просмотра. Официальные спортивные расчёты EFRA не менялись.</p></div><div class="classicResultsDesktop"><div class="tableWrap"><table class="table classicResultsTable"><thead><tr><th>POS</th><th>ПИЛОТ</th><th>FINAL</th><th>BEST 2</th></tr></thead><tbody>${desktopRows||'<tr><td colspan="4">Итоговый протокол ещё не сформирован.</td></tr>'}</tbody></table></div></div><div class="classicResultsMobile">${cards||'<div class="rxnEmpty">Итоговый протокол ещё не сформирован.</div>'}</div></section>`;}
function classicRCLapChips(c,ev,pilotId){const times=ev?.lapStats?.[pilotId]?.lapTimes||[];if(!times.length)return'<span class="classicCellNote">Нет записанных кругов</span>';const window=ev.stage==='seeding'?classicRCSeedWindow(c,pilotId):null;return `<div class="classicLapChips">${times.map((ms,i)=>{const lap=i+1,linked=window&&window.eventKey===ev.key&&lap>=window.startIndex&&lap<=window.endIndex;return `<span class="classicLapChip ${linked?'good':''}"><small>L${lap}</small><b>${rxnFormatDuration(ms)}</b></span>`;}).join('')}</div>`;}
...
function classicRCHeatProtocolsSection(c){
  ...
  const blocks=completed.map(ev=>{
    const eventBest=Math.min(...(ev.pilots||[]).map(id=>Number(ev.lapStats?.[id]?.bestLapMs)).filter(x=>Number.isFinite(x)&&x>0),Infinity),resultById=new Map((ev.result||[]).map((r,i)=>[String(r.pilotId),{...r,_index:i}]));
    ...
    const desktopRows=ordered.map(id=>{const pilot=ClassicRCEngine.pilot(id),r=resultById.get(String(id))||{},st=ev.lapStats?.[id]||{},status=String(r.status||'FIN'),place=status==='FIN'?(r.place??(Number.isFinite(r._index)?r._index+1:'—')):status,best=Number(st.bestLapMs),avg=classicRCLapAverageMs(st),isEventBest=Number.isFinite(best)&&best===eventBest,counted=classicRCEventCounted(c,ev,id),points=classicRCEventPoints(c,ev,id);return `<tr><td><b>${esc(place)}</b></td><td><b>${esc(rxnPilotDisplayName(pilot))}</b></td><td>${counted.counted?`<span class="classicResultBadge good">${esc(counted.label)}</span>`:'—'}</td><td>${Number(r.laps??st.laps)||0}</td><td>${Number(r.timeMs)>0?rxnFormatDuration(r.timeMs):'—'}</td><td class="${isEventBest?'classicRunBest':''}">${Number.isFinite(best)&&best>0?rxnFormatDuration(best):'—'}</td><td>${avg?rxnFormatDuration(avg):'—'}</td><td>${points}</td><td><details class="classicLapDetails"><summary>КРУГИ</summary>${classicRCLapChips(c,ev,id)}</details></td></tr>`;}).join('');
    const cards=ordered.map(id=>{const pilot=ClassicRCEngine.pilot(id),r=resultById.get(String(id))||{},st=ev.lapStats?.[id]||{},status=String(r.status||'FIN'),place=status==='FIN'?(r.place??(Number.isFinite(r._index)?r._index+1:'—')):status,best=Number(st.bestLapMs),avg=classicRCLapAverageMs(st),isEventBest=Number.isFinite(best)&&best===eventBest,counted=classicRCEventCounted(c,ev,id),points=classicRCEventPoints(c,ev,id),seed=ev.stage==='seeding'?classicRCSeedWindow(c,id):null;return `<article class="classicRunCard"><div class="classicRunCardHead"><strong>${esc(place)}</strong><b>${esc(rxnPilotDisplayName(pilot))}</b>${counted.counted?`<span class="classicResultBadge good">${esc(counted.label)}</span>`:''}</div>${seed&&seed.eventKey===ev.key?`<div class="classicRunSeedBest"><small>ЛУЧШАЯ СВЯЗКА</small><b>${rxnFormatDuration(seed.avgMs)} AVG</b><span>КРУГИ ${seed.startIndex}–${seed.endIndex}</span></div>`:''}<div class="classicRunMetrics"><span><small>КРУГИ</small><b>${Number(r.laps??st.laps)||0}</b></span><span><small>ВРЕМЯ</small><b>${Number(r.timeMs)>0?rxnFormatDuration(r.timeMs):'—'}</b></span><span class="${isEventBest?'good':''}"><small>BEST</small><b>${Number.isFinite(best)&&best>0?rxnFormatDuration(best):'—'}</b></span><span><small>AVG</small><b>${avg?rxnFormatDuration(avg):'—'}</b></span>${points!=='—'?`<span><small>ОЧКИ</small><b>${points}</b></span>`:''}</div><details class="classicLapDetails"><summary>КРУГИ ПОДРОБНО</summary>${classicRCLapChips(c,ev,id)}</details></article>`;}).join('');
    return `<details class="classicHeatProtocol"><summary><span>${esc(ClassicRCEngine.stageLabel(ev.stage))}</span><b>${esc(ev.label)}</b><em>${(ev.pilots||[]).length} PILOTS</em></summary><div class="classicHeatProtocolBody"><div class="classicResultsDesktop"><div class="tableWrap"><table class="table classicRunTable"><thead><tr><th>POS</th><th>ПИЛОТ</th><th>ЗАЧЁТ</th><th>КРУГИ</th><th>ВРЕМЯ</th><th>BEST</th><th>AVG</th><th>ОЧКИ</th><th>ДЕТАЛИ</th></tr></thead><tbody>${desktopRows}</tbody></table></div></div><div class="classicResultsMobile">${cards}</div></div></details>`;
  }).join('');
  return `<section class="classicResultsSection classicHeatProtocols"><div class="classicResultsSectionHead"><h3>ПРОТОКОЛЫ ЗАЕЗДОВ</h3><p>Как в MyRCM: сначала итог заезда — место, круги, время, BEST, AVG и очки. Покруговая раскладка открывается отдельно.</p></div>${blocks}</section>`;
}
function classicRCResultsContent(includeOverall=false,includeHeats=false){const c=ClassicRCEngine.get();if(!c)return'';const q=c.qualifyingStandings||[],rounds=Number(c.settings.qualifyingRounds||5),seed=c.seedStandings||[],finals=c.finalStandings||[];return `${classicRCResultsOverview(c)}${includeOverall?classicRCOverallProtocolSection(c):''}${classicRCSeedingSection(c,seed)}${classicRCQualifyingSection(c,q,rounds)}${classicRCFinalsSection(c,finals)}${includeHeats?classicRCHeatProtocolsSection(c):''}`;}
function classicRCResultsModal(){const c=ClassicRCEngine.get();if(!c)return;$('#modalHost').innerHTML=`<div class="modalBackdrop"><div class="modal classicTablesModal"><div class="modalHead"><div><div class="sectionLabel">CLASSIC RC · ${esc(c.ruleset)}</div><h2>${esc(c.name)}</h2><p>Удобное отображение протоколов без изменения официальной спортивной логики EFRA.</p></div><button class="iconBtn" data-classic-modal-close>×</button></div><div class="classicRulesBar"><span>SEEDING · BEST ${c.settings.seedingConsecutiveLaps} AVG</span><span>QUALIFYING · ${Number(c.settings.qualifyingRounds||5)} ROUND(S)</span><span>В ЗАЧЁТ · ${classicRCQualifyingRoundsToCount(Number(c.settings.qualifyingRounds||5))}</span><span>FINALS · 3 LEGS · BEST 2</span><span>MAX 10 / HEAT</span></div>${classicRCResultsContent(true,true)}</div></div>`;bindClassicRC();}
function classicRCFinalProtocolMarkup(){const c=ClassicRCEngine.get();if(!c)return'<div class="rxnEmpty">Итоговый протокол ещё не сформирован.</div>';return `<div class="classicResultsBoard">${classicRCResultsOverview(c)}${classicRCOverallProtocolSection(c)}${classicRCFinalsSection(c,c.finalStandings||[])}</div>`;}
