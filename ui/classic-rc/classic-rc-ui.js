...
function classicRCStageBanner(ev){
  ...
  if(!ev)return{stage:'CLASSIC RC',heat:'—',discipline:'EFRA 2026',pilots:'ПИЛОТЫ 0/0'};
  if(ev.stage==='seeding')return{stage:`ПОСЕВ ${ev.round}`,heat:`ГРУППА ${ev.heat}/${groups}`,discipline:'CLASSIC RC',pilots:`ПИЛОТЫ ${count}/${count}`};
  if(ev.stage==='controlled')return{stage:`ПРАКТИКА ${ev.round}`,heat:`ГРУППА ${ev.heat}/${groups}`,discipline:'CLASSIC RC',pilots:`ПИЛОТЫ ${count}/${count}`};
  if(ev.stage==='qualifying')return{stage:`КВ${ev.round}`,heat:`ГРУППА ${ev.heat}/${groups}`,discipline:'CLASSIC RC',pilots:`ПИЛОТЫ ${count}/${count}`};
  if(ev.stage==='finalPractice')return{stage:`ФИНАЛ ${ev.groupName}`,heat:'ПРАКТИКА',discipline:'CLASSIC RC',pilots:`ПИЛОТЫ ${count}/${count}`};
  return{stage:`ФИНАЛ ${ev.groupName}`,heat:`ЗАЕЗД ${ev.round}/3`,discipline:'CLASSIC RC',pilots:`ПИЛОТЫ ${count}/${count}`};
}
...
function classicRCAutoDayControls(){
  const c=ClassicRCEngine.get(),st=ClassicRCRuntime.autoState?.();if(!c?.timeline||c.status==='finished')return'';
...
function classicRCCockpitView(){rxnEnsureSystemClockTicker();classicRCEnsureScheduleTicker();ClassicRCRuntime.ensureAutoTicker?.();const c=ClassicRCEngine.get();if(!c||c.status==='setup')return `<section class="page"><div class="card"><h2>Classic RC ещё не подготовлен</h2><button class="btn primary" data-classic-action="setup">К НАСТРОЙКЕ</button></div></section>`;const ev=ClassicRCRuntime.event(),cls=rxnColumnClass(),count=rxnMetricCount();if(!ev&&c.status==='finished')return `<section class="rxnCockpit classicRCCockpit classicResultsCockpit ${cls}" style="--rxn-metric-count:${count}">${classicRCHeader()}${classicRCTitle()}<main class="classicResultsMain"><section class="classicResultsRoster"><div class="classicFinishedActions"><button class="btn primary" data-classic-action="results">ВСЕ РЕЗУЛЬТАТЫ</button><button class="btn secondary" data-classic-action="schedule">РАСПИСАНИЕ</button><button class="btn secondary" data-classic-action="race-settings">НАСТРОЙКИ</button><button class="btn secondary" data-classic-action="home">ГЛАВНАЯ</button><button class="btn danger" data-classic-action="archive">ЗАВЕРШИТЬ</button></div><div class="classicFinalProtocol">${classicRCFinalProtocolMarkup()}</div></section></main></section>`;ClassicRCRuntime.ensureSession();return `<section class="rxnCockpit classicRCCockpit ${cls}" style="--rxn-metric-count:${count}">${classicRCHeader()}${classicRCTitle()}<main class="rxnMain"><section class="rxnRoster"><div id="classicPilotBoard" class="rxnTable">${classicRCPilotTable()}</div></section><aside class="rxnSide">${classicRCScheduleStatusStrip(true)}${classicRCTimerPanel()}${classicRCMobileInfo()}${classicRCControls()}</aside></main></section>`;}
...
function classicRCStageRu(stage){return({seeding:'ПОСЕВ',controlled:'КОНТРОЛЬНАЯ ПРАКТИКА',qualifying:'КВАЛИФИКАЦИЯ',finalPractice:'ПРАКТИКА ПЕРЕД ФИНАЛОМ',final:'ФИНАЛ'})[stage]||String(stage||'').toUpperCase();}
...
function classicRCSeedingSection(c,seed){
  ...
  return `<section class="classicResultsSection"><div class="classicProtocolTitle"><h3>ПОСЕВ</h3><span>ЛУЧШАЯ СВЯЗКА ИЗ ${c.settings.seedingConsecutiveLaps} КРУГОВ ПОДРЯД</span></div><div class="classicProtocolScroll"><table class="table classicProtocolTable classicProtocolStandings"><thead><tr><th>МЕСТО</th><th>ПИЛОТ</th><th>СРЕДНЕЕ СВЯЗКИ</th><th>КРУГИ</th><th>ЗАЕЗД</th></tr></thead><tbody>${rows||'<tr><td colspan="5">НЕТ ДАННЫХ</td></tr>'}</tbody></table></div></section>`;
}
