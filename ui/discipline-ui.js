...
function rxnPilotTable(pilots,s){
  ...
  const rows=pilots.map((p,i)=>{
    const l=s?.live?.[p.id]||blankLive(),st=rxnRowState(s,l,p.id,i),avg=rxnLapAvg(l),progress=rxnProgress(l,s),hot=Number.isFinite(l.lastPassPerf)&&performance.now()-l.lastPassPerf<800;
...
function rxnRaceBannerData(race,ev,pilots=[]){
  ...
  if(ev.tieBreak)return{stage:'ПЕРЕЗАЕЗД',heat:'HEAT 1/1',discipline:'RALLYCROSS',pilots:`PILOTS ${count}/${Math.max(count,capacity)}`};
...
function rxnControlGrid(done,s,tie=false){
  if(tie)return `<div class="rxnControls">${rxnControlButton('blue','data-action="race-results"','chart','ТАБЛИЦА','РЕЗУЛЬТАТЫ')}${rxnControlButton('','disabled','pause','ПАУЗА')}${rxnControlButton('','disabled','flag','ФИНИШ')}${rxnControlButton('blue','disabled','refresh','РЕСТАРТ')}${rxnControlButton('primary','data-action="tie-runoff"','refresh','ПЕРЕЗАЕЗД')}${rxnControlButton('danger','data-action="home"','stop','ВЫХОД')}</div>`;
  if(done)return `<div class="rxnControls">${rxnControlButton('blue','data-action="race-results"','chart','РЕЗУЛЬТАТЫ')}${rxnControlButton('','data-action="home"','home','ГЛАВНАЯ')}${rxnControlButton('','data-action="open-rx"','settings','НАСТРОЙКА')}${rxnControlButton('blue','disabled','refresh','РЕСТАРТ')}${rxnControlButton('blue','disabled','settings','НАСТРОЙКИ','СЕССИИ')}${rxnControlButton('danger','data-action="complete-competition"','stop','ЗАВЕРШИТЬ')}</div>`;
  const p=s?.phase||'ready',settingsEnabled=['ready','finished'].includes(p),restartEnabled=p==='finished';
  ...
  return `<div class="rxnControls">${primary}${rxnControlButton('','data-action="pause-session" '+(!['running','finishing'].includes(p)?'disabled':''),'pause','ПАУЗА')}${rxnControlButton('','data-action="finish-session" '+(!['running','finishing','paused'].includes(p)?'disabled':''),'flag','ФИНИШ')}${rxnControlButton('blue','data-action="restart-session" '+(!restartEnabled?'disabled':''),'refresh','РЕСТАРТ')}${rxnControlButton('blue','data-action="session-settings" '+(!settingsEnabled?'disabled':''),'settings','НАСТРОЙКИ','СЕССИИ')}${rxnControlButton('danger','data-action="stop-session" '+(!['warmup','countdown','running','finishing','paused'].includes(p)?'disabled':''),'stop','СТОП')}</div>`;
}
...
function rxnResultConfirmModal(){
  ...
  $('#modalHost').innerHTML=`<div class="modalBackdrop"><div class="modal rxnResultModal"><div class="modalHead"><div><div class="sectionLabel">${isTie?'ПЕРЕЗАЕЗД':isQ?'КВАЛИФИКАЦИЯ':'ФИНАЛ'} · ПОДТВЕРЖДЕНИЕ</div><h2>${esc(ev.label||eventShortLabel(ev))}</h2><p>Порядок предложен по засечке. Перед сохранением проверьте FIN / DNF / DNS / DSQ.</p></div><button class="iconBtn" data-rxn-result-close="1">×</button></div>
    <div class="rxnResultRuleNote">${isTie?'Перезаезд определяет только порядок спорных мест. Дополнительные очки не начисляются.':isQ?`Очки Q: ${rxnRuleView().qualifyingPointsPreview.join(' · ')}… · BEST ${rxnRuleView().qualifyingBestCount}`:`Финал A: ${rxnRuleView().finalRuns.join(' / ')} · учитываются лучшие ${rxnRuleView().finalBestCount} · DNF/DNS/DSQ = ${rxnRuleView().finalNonFinishScore} для суммы финала`}</div>
...
function rxnStartGridOrder(race,raw){return RallyCrossModeAPI.startGrid(race,raw);}
...
function updateTrackDayDynamicUI(){
...
document.addEventListener('click',e=>{
  ...
  const confirmGrid=e.target.closest?.('[data-rxn-grid-confirm]');if(confirmGrid){const raw=findRawEvent(confirmGrid.dataset.rxnGridConfirm);if(raw){state.race.startGrids=state.race.startGrids||{};state.race.startGrids[raw.key]={order:rxnStartGridOrder(state.race,raw).map(x=>x.pilot.id),confirmedAt:new Date().toISOString()};persistRace();toast('Стартовая решётка подтверждена');}closeModal();return;}
});
...
AppBridge.updateRace=updateDynamicCockpitUI;AppBridge.updatePractice=updateTrackDayDynamicUI;

AppBridge.openRaceResultEditor=rxnResultConfirmModal;
