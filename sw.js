/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC76 · EFRA DIRECTOR + PARTICIPANTS */
'use strict';
const LEGION_SW_BUILD='rc76-efra-director-participants';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc76-efra-director-participants');
...
async function validate(url,response){
  ...
  if(url==='./modes/classic-rc/efra-engine.js'){const s=await textOf(response);if(!s.includes('legionrx4_classic_rc_efra_event_v1')||!s.includes('buildFinals')||!s.includes('CompetitionScheduler')||!s.includes('updateCurrentHeatSettings')||!s.includes('pauseCompetition')||!s.includes('shiftScheduleStart')||!s.includes('restartCurrentEvent')||!s.includes('CompetitionScheduler.skipHeat')||!s.includes('abortCompetition')||!s.includes('lateEntry')||!s.includes('withdrawPilot')||!s.includes('staggerIntervalMs'))throw new Error(`${url}: stale Classic RC engine`);}
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc76-efra-director-participants')||!s.includes('./runtime/competition-autopilot.js')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC76 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC76')||!s.includes('EFRA DIRECTOR + PARTICIPANTS'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
