/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC77 · STAGGER + PARTICIPANT UI */
'use strict';
const LEGION_SW_BUILD='rc77-stagger-participant-ui';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc77-stagger-participant-ui');
...
async function validate(url,response){
  ...
  if(url==='./ui/shell/router.js'){const s=await textOf(response);if(!s.includes('homeViewClassic')||!s.includes('homeViewRxui')||!s.includes('classicCockpit')||!s.includes('classicLive')||!s.includes('EFRA 2026'))throw new Error(`${url}: stale Classic RC router/home`);}
  ...
  if(url==='./ui/pilots/pilot-cards.js'){const s=await textOf(response);if(!s.includes('pilotRaceEntryForProfile')||!s.includes('updateActiveRacePilotIdentity')||!s.includes('raceModel')||!s.includes('options.onSave'))throw new Error(`${url}: stale pilot profile live-sync`);}
  ...
  if(url==='./modes/classic-rc/index.js'){const s=await textOf(response);if(!s.includes("label:'EFRA RC'")||!s.includes("id:'classic-rc'"))throw new Error(`${url}: stale EFRA RC mode label`);}
  ...
  if(url==='./modes/classic-rc/efra-engine.js'){const s=await textOf(response);if(!s.includes('legionrx4_classic_rc_efra_event_v1')||!s.includes('buildFinals')||!s.includes('CompetitionScheduler')||!s.includes('updateCurrentHeatSettings')||!s.includes('pauseCompetition')||!s.includes('shiftScheduleStart')||!s.includes('restartCurrentEvent')||!s.includes('CompetitionScheduler.skipHeat')||!s.includes('abortCompetition')||!s.includes('lateEntry')||!s.includes('withdrawPilot')||!s.includes('staggerIntervalMs')||!s.includes('staggerReleaseV2'))throw new Error(`${url}: stale Classic RC engine`);}
  if(url==='./modes/classic-rc/audio-actions.js'){const s=await textOf(response);if(!s.includes('const ClassicRCAudio')||!s.includes('announcer.playBleep')||!s.includes('pilotVoices.play')||!s.includes('timeExpired')||!s.includes('heatResults')||!s.includes('staggerRelease')||!s.includes('staggerPilotCall')||s.includes('getEventStartPilots')||s.includes('state.race?.'))throw new Error(`${url}: stale/coupled Classic RC audio adapter`);}
  if(url==='./modes/classic-rc/efra-runtime.js'){const s=await textOf(response);if(!s.includes('const ClassicRCRuntime')||!s.includes("ev.stage==='qualifying'")||!s.includes('canStartScheduleHeat')||!s.includes('rawNowEpoch')||!s.includes('cycleSimulationSpeed')||!s.includes('restartCurrentEvent')||!s.includes('ClassicRCAudio.countdownTick')||!s.includes('ClassicRCAudio.bleep')||!s.includes('ClassicRCAudio.bestLap')||!s.includes('startAutoDay')||!s.includes('CompetitionAutoPilot.nextAction')||!s.includes('staggerReleaseStep')||!s.includes('ClassicRCAudio.staggerRelease')||!s.includes('abortCompetition')||!s.includes("ev.startMode==='staggered'")||!s.includes('afterHorn:true')||!s.includes('preCountdownLeadMs')||s.includes('raceSimulator')||s.includes('rxnFormatDuration')||s.includes('document.'))throw new Error(`${url}: stale/coupled Classic RC runtime`);}
  if(url==='./ui/classic-rc/classic-rc-ui.js'){const s=await textOf(response);if(!s.includes('classicRCScheduleDrawer')||!s.includes("current?.status||''}:${current?.kind||''}:${snap.hold}")||!s.includes('classicRCScheduleStatusStrip')||!s.includes('classicScheduleNext')||!s.includes('Math.ceil((Number(ms)||0)/1000)')||!s.includes('НАЧАТЬ СЕЙЧАС')||!s.includes('ОСТАНОВИТЬ СОРЕВНОВАНИЕ')||!s.includes('СТАРТ ДНЯ')||!s.includes('РЕСТАРТ ЗАЕЗДА')||!s.includes('classicScheduleInlineAdjust')||!s.includes('classicRCAutoDayControls')||!s.includes('auto-start-day')||!s.includes('sim-speed')||!s.includes('classicRCBindActionElement')||!s.includes("dataset.classicBound='1'")||!s.includes('preserveScroll=true')||!s.includes('classicRCHeatProtocolsSection')||!s.includes('classicRCLapMatrix')||!s.includes('РЕЗУЛЬТАТ ЗАЕЗДА')||!s.includes('directorOverride:true')||!s.includes('classicRCHeatProtocolsSection')||!s.includes('classicResultsCockpit')||!s.includes('classicRCRaceSettingsModal')||!s.includes('classicFinalRankRow')||!s.includes('ИТОГОВЫЙ РЕЙТИНГ ФИНАЛОВ')||!s.includes('classicRCParticipantsDrawer')||!s.includes('classicRCStopOverlay')||!s.includes('classicRCStaggerPanel')||!s.includes('classicRCFinalGridPanel')||!s.includes('competition-abort-confirm')||!s.includes('classicParticipantPortrait')||s.includes('classicRCEnsureDelegatedBindings')||!s.includes('EFRA 2026'))throw new Error(`${url}: stale Classic RC UI`);}
  if(url==='./ui/classic-rc/classic-rc.css'){const s=await textOf(response);if(!s.includes('.classicScheduleDrawer')||!s.includes('orientation:portrait')||!s.includes('#classicScheduleHost{')||!s.includes('position:fixed;inset:0')||!s.includes('grid-template-rows:38px minmax(145px,34%) minmax(0,1fr)')||!s.includes('.classicScheduleClock')||!s.includes('.classicScheduleDayAdjust')||!s.includes('.classicScheduleInlineAdjust')||!s.includes('.classicResultsCockpit')||!s.includes('.classicHeatProtocol')||!s.includes('.classicRunCard')||!s.includes('.classicLapMatrix')||!s.includes('.classicProtocolTable')||!s.includes('.classicRaceSettingsModal')||!s.includes('.classicFinalRankRow')||!s.includes('.classicParticipantsDrawer')||!s.includes('.classicStopOverlay')||!s.includes('.classicStaggerPanel')||!s.includes('.classicFinalGridPanel')||!s.includes('.classicParticipantPortrait')||!s.includes('bottom:125px')||!s.includes('position:absolute!important')||!s.includes('.classicScheduleTail.open{opacity:0')||s.includes('.classicScheduleDrawer button,.classicScheduleTail{\n  position:relative'))throw new Error(`${url}: stale schedule portal/status style`);}
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc77-stagger-participant-ui')||!s.includes('./runtime/competition-autopilot.js')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC77 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC77')||!s.includes('STAGGER + PARTICIPANT UI'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
