/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC70 · SINGLE CLASS DIRECTOR */
'use strict';
const LEGION_SW_BUILD='rc70-single-class-director';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc70-single-class-director');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc70-single-class-director')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC70 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC70')||!s.includes('SINGLE CLASS DIRECTOR'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
