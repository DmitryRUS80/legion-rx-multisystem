/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC66 · CLASSIC SCHEDULE OVERLAY FIX */
'use strict';
const LEGION_SW_BUILD='rc66-classic-schedule-overlay-fix';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc66-classic-schedule-overlay-fix');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc66-classic-schedule-overlay-fix')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC66 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC66')||!s.includes('CLASSIC SCHEDULE OVERLAY FIX'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
