...
'use strict';
const LEGION_SW_BUILD='rc74-classic-audio-final-rating';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc74-classic-audio-final-rating');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc74-classic-audio-final-rating')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC74 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC74')||!s.includes('CLASSIC AUDIO + FINAL RATING'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
