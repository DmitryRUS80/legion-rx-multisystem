/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC67 · CLASSIC SCHEDULE PORTAL HIT FIX */
'use strict';
const LEGION_SW_BUILD='rc67-classic-schedule-portal-hit-fix';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc67-classic-schedule-portal-hit-fix');
...
async function validate(url,response){
  ...
  if(url==='./runtime/competition-scheduler.js'){const s=await textOf(response);if(!s.includes('const CompetitionScheduler')||!s.includes('const canStart=')||!s.includes('const finish=')||!s.includes('adjustBreakMinutes')||!s.includes('setHeatDuration')||s.includes('RallyCross')||s.includes('ClassicRC'))throw new Error(`${url}: stale/non-neutral competition scheduler`);}
  ...
  if(url==='./ui/classic-rc/classic-rc.css'){const s=await textOf(response);if(!s.includes('.classicScheduleDrawer')||!s.includes('orientation:portrait')||!s.includes('position:fixed')||!s.includes('grid-template-rows:38px minmax(145px,34%) minmax(0,1fr)')||!s.includes('.classicScheduleClock')||!s.includes('position:fixed!important')||!s.includes('.classicScheduleTail.open{opacity:0')||s.includes('.classicScheduleDrawer button,.classicScheduleTail{\n  position:relative'))throw new Error(`${url}: stale schedule overlay/status style`);}
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc67-classic-schedule-portal-hit-fix')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC67 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC67')||!s.includes('CLASSIC SCHEDULE PORTAL HIT FIX'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
