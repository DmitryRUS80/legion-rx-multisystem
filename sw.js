/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC73 · REPORT STYLE PROTOCOLS */
'use strict';
const LEGION_SW_BUILD='rc73-report-style-protocols';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc73-report-style-protocols');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc73-report-style-protocols')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC73 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC73')||!s.includes('REPORT STYLE PROTOCOLS'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
