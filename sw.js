/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC62 · CLASSIC RC EFRA + SCHEDULER */
'use strict';
const LEGION_SW_BUILD='rc62-classic-rc-efra-scheduler';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc62-classic-rc-efra-scheduler');
...
const FRESH_REQUIRED=new Set(['./','./index.html','./VERSION.txt','./offline-manifest.js','./app.js','./boot.js','./platform/updater.js','./platform/pilot-live-edit.js','./runtime/competition-scheduler.js','./runtime/race-event-bus.js','./runtime/race-clock-adapter.js','./runtime/race-test-source-adapter.js','./runtime/active-race-controller.js','./modes/classic-rc/efra-rules.js','./modes/classic-rc/groups.js','./modes/classic-rc/efra-engine.js','./modes/classic-rc/efra-runtime.js','./modes/rallycross/index.js','./modes/rallycross/finals.js','./modes/rallycross/runtime.js','./simulation/race-simulator.js','./ui/pilots/pilot-cards.js','./ui/classic-rc/classic-rc-ui.js','./ui/classic-rc/classic-rc.css','./ui/shell/views.js','./ui/shell/router.js','./ui/shell/actions.js','./ui/shell/offline-runtime.js','./ui/shell/app.css','./ui/discipline-ui.js','./ui/shell/discipline-pults.css','./ui/skins/rxui/base.css','./ui/skins/rxui/cobalt.css','./ui/skins/rxui/workspace-landscape.webp','./ui/skins/rxui/workspace-portrait.webp','./ui/classic-polish/classic-controls.css','./ui/classic-polish/classic-icons.js']);
...
async function validate(url,response){
  ...
  if(url==='./runtime/competition-scheduler.js'){const s=await textOf(response);if(!s.includes('const CompetitionScheduler')||!s.includes('const canStart=')||!s.includes('const finish=')||s.includes('RallyCross')||s.includes('ClassicRC'))throw new Error(`${url}: stale/non-neutral competition scheduler`);}
  if(url==='./runtime/active-race-controller.js'){const s=await textOf(response);if(!s.includes('ClassicRCRuntime.processPass')||!s.includes('processPass(detail.transponder'))throw new Error(`${url}: stale active race controller`);}
  ...
  if(url==='./modes/classic-rc/efra-engine.js'){const s=await textOf(response);if(!s.includes('legionrx4_classic_rc_efra_event_v1')||!s.includes('buildFinals')||!s.includes('CompetitionScheduler'))throw new Error(`${url}: stale Classic RC engine`);}
  if(url==='./modes/classic-rc/efra-runtime.js'){const s=await textOf(response);if(!s.includes('const ClassicRCRuntime')||!s.includes("ev.stage==='qualifying'")||!s.includes('canStartScheduleHeat')||!s.includes("['seeding','controlled','finalPractice','qualifying'].includes(ev.stage)")||s.includes('rxnFormatDuration')||s.includes('document.'))throw new Error(`${url}: stale/coupled Classic RC runtime`);}
  if(url==='./ui/classic-rc/classic-rc-ui.js'){const s=await textOf(response);if(!s.includes('classicRCScheduleDrawer')||!s.includes('НАЧАТЬ РАНЬШЕ')||!s.includes('EFRA 2026'))throw new Error(`${url}: stale Classic RC UI`);}
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc62-classic-rc-efra-scheduler')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC62 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC62')||!s.includes('CLASSIC RC EFRA'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
