/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC71 · CLASSIC RESULTS UX */
'use strict';
const LEGION_SW_BUILD='rc71-classic-results-ux';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc71-classic-results-ux');
...
async function validate(url,response){
  ...
  if(url==='./runtime/competition-scheduler.js'){const s=await textOf(response);if(!s.includes('const CompetitionScheduler')||!s.includes('const canStart=')||!s.includes('const finish=')||!s.includes('adjustBreakMinutes')||!s.includes('setHeatDuration')||!s.includes('shiftStart')||!s.includes('restartHeat')||s.includes('RallyCross')||s.includes('ClassicRC'))throw new Error(`${url}: stale/non-neutral competition scheduler`);}
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc71-classic-results-ux')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC71 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC71')||!s.includes('CLASSIC RESULTS UX'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
