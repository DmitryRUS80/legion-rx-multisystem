/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC72 · SCHEDULE + PROTOCOL UX */
'use strict';
const LEGION_SW_BUILD='rc72-schedule-protocol-ux';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc72-schedule-protocol-ux');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc72-schedule-protocol-ux')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC72 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC72')||!s.includes('SCHEDULE + PROTOCOL UX'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
