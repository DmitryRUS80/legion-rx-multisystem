/* LEGION RX · SAFE OFFLINE SERVICE WORKER · RC65 · CLASSIC SCHEDULE BUTTON FIX */
'use strict';
const LEGION_SW_BUILD='rc65-classic-schedule-button-fix';
/* Unique import URL prevents an older HTTP-cached release manifest being reused during SW update. */
importScripts('./offline-manifest.js?build=rc65-classic-schedule-button-fix');
...
async function validate(url,response){
  ...
  if(url==='./offline-manifest.js'){const s=await textOf(response);if(!s.includes('4.2.0-clean-full-rc65-classic-schedule-button-fix')||!s.includes('./runtime/race-event-bus.js'))throw new Error(`${url}: stale RC65 release manifest`);}
  if(url==='./VERSION.txt'){const s=await textOf(response);if(!s.includes('RC65')||!s.includes('CLASSIC SCHEDULE BUTTON FIX'))throw new Error(`${url}: stale VERSION`);}
  return response;
}
